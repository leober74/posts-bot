...
// 🔥 НОВОЕ: Тестовый платёж на 1 рубль
async function createTestPaymentLink(telegramId) {
  console.log('🧪 Создаём тестовую оплату 1 рубль для:', telegramId);
  return createPaymentLink(telegramId, 'Тестовый платёж', 1);
}
...
async function registerWebhook(botUrl) {
  try {
    ...
    console.log('🔄 Регистрируем webhook:', webhookUrl); // 🔥 ЛОГ 1
    ...
    console.log('✅ Webhook зарегистрирован!', response.data); // 🔥 ЛОГ 2
    return response.data;
  } catch (e) {
    console.error('❌ Ошибка регистрации webhook:', e.response?.data || e.message); // 🔥 ЛОГ 3
  }
}

async function createPaymentLink(telegramId, purpose = 'Подписка на бота (1 месяц)', amount = 100) {
  try {
    console.log(`💳 Создаём платёж ${amount}р для ${telegramId}`); // 🔥 ЛОГ
    ...
    console.log(`✅ Ссылка создана: ${link ? 'ДА' : 'НЕТ'}`);
    return link || null;
  } catch (e) {
...
async function handlePaymentWebhook(bot, jwtToken) {
  try {
    console.log('📥 Webhook от Точки получен!'); // 🔥 ЛОГ 4
    ...
    console.log('🔓 JWT декодирован:', JSON.stringify(payload)); // 🔥 ЛОГ 5
    ...
    console.log(`✅ Подписка активирована для ${telegramId}`); // 🔥 ЛОГ 6
