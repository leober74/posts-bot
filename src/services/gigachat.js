...
6. Вопрос который хочется обсудить`
  ...
  // Умная логика ссылки — зависит от типа поста
  let botLinkInstruction = '';
  if (postType === 'продающий') {
    botLinkInstruction = `ОБЯЗАТЕЛЬНО: последняя строка поста — короткое нативное приглашение попробовать бота. Например: "P.S. Этот пост написан с помощью бота за 3 минуты → ${botLink}" или "Хочешь писать посты так же быстро? → ${botLink}" — подбери вариант под контекст.`;
  } else if (postType === 'развлекательный') {
    botLinkInstruction = `ОБЯЗАТЕЛЬНО: последняя строка поста — лёгкое, ненавязчивое упоминание бота в тему. Например: "Кстати, этот пост сгенерировал бот → ${botLink}" или "P.S. Написано за 3 минуты вот здесь → ${botLink}"`;
  } else if (postType === 'экспертный') {
    botLinkInstruction = `ОПЦИОНАЛЬНО: если по смыслу уместно — добавь в конце нативную строку про бота (${botLink}). Если пост заканчивается сильным выводом или советом — НЕ добавляй ссылку, не ломай финал.`;
  } else if (postType === 'вовлекающий') {
    botLinkInstruction = `НЕ добавляй ссылку на бота в конце. Вовлекающий пост должен заканчиваться вопросом к аудитории — ссылка здесь выглядит чужеродно и убивает вовлечение. Финал — только вопрос.`;
  }
...
- НИКАКИХ выдуманных материалов: не упоминай чеклисты, PDF, гайды, пошаговые инструкции, записи, таблицы, файлы — ничего что автор не сможет дать реально. Призыв к действию только реальный: написать в личку, подписаться, оставить комментарий
...
async function generatePost(postType, userData, regenCount = 0) {
  ...
  const response = await axios.post(
    'https://gigachat.devices.sberbank.ru/api/v1/chat/completions',
    {
      ...
      temperature: 0.85 + (regenCount * 0.05),
      max_tokens: 1000
    },
    {
      headers: {
